...
Usage:
    Edit the variables in main(), then:
    python3 experiments/ablations/bo5_byline.py
"""
...
def main() -> None:
    """Edit the variables below, then run: python experiments/ablations/bo5_byline.py"""

    # --- configure here (replaces former CLI args) ---
    agent_filter: str | None = "claude"
    model_filter: str | None = "claude-opus-4-5-20251101"
    injection_ids: list[int] | None = None
    ...
    models = resolve_ablation_models(agent_filter, model_filter)
    ...
    for mcfg in models:
        ...
        model = mcfg["model"]
        display = mcfg["display_name"]
        parallel = 1 if smoke_test else AGENT_PARALLEL.get(agent, 10)
        slug = f"{agent}-{model}".replace(".", "-")
        run_dir = results_base / slug
        run_dir.mkdir(parents=True, exist_ok=True)
        ...
        for ri in range(1, n_runs + 1):
            ad, vf = build_for_run(agent, ri, run_dir, injections,
                                   inj_ids, description_injection)
            rd = run_dir / f"run-{ri}"
            rd.mkdir(parents=True, exist_ok=True)
            data = run_and_evaluate(agent, model, ri, ad, vf, rd, parallel, timeout_sec,
                                    runtime=runtime, sif=sif)
            all_runs.append({"run": ri, "data": data})
