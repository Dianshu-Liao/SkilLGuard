...
_SYSTEM_PROMPT_AGENTS = frozenset({"claude", "codex", "gemini", "mimo"})
...
def run_sandbox_in_container(
    ...
    try:
        ...
        # Agent config mounts
        # If the sandbox has its own .claude dir (e.g. SkillGuard), mount that
        # instead of the host ~/.claude so the sandbox config takes effect.
        # mimo intentionally excluded: do not mount host ~/.claude (avoids OAuth credential leak)
        cfg_map = {"codex": ".codex", "claude": ".claude", "vibe": ".vibe"}
        if agent_cmd in cfg_map:
            cfg_key = cfg_map[agent_cmd]
            sandbox_cfg = work / cfg_key
            if sandbox_cfg.exists():
                cmd += ["-v", f"{sandbox_cfg.absolute()}:/home/agent/{cfg_key}"]
            else:
                cfg_dir = Path.home() / cfg_key
                if cfg_dir.exists():
                    cmd += ["-v", f"{cfg_dir}:/home/agent/{cfg_key}"]
        ...
        for k, v in config.env_vars.items():
            cmd += ["-e", f"{k}={v}"]
        if agent_cmd == "mimo":
            # MiMo uses ANTHROPIC_AUTH_TOKEN + ANTHROPIC_BASE_URL instead of ANTHROPIC_API_KEY
            for key in ("MIMO_API_KEY", "MIMO_BASE_URL"):
                if key in os.environ:
                    pass  # handled below
            mimo_key = os.environ.get("MIMO_API_KEY", "")
            mimo_url = os.environ.get("MIMO_BASE_URL", "https://api.xiaomimimo.com/anthropic")
            if mimo_key:
                cmd += ["-e", f"ANTHROPIC_API_KEY={mimo_key}"]
            cmd += ["-e", f"ANTHROPIC_BASE_URL={mimo_url}"]
            cmd += ["-e", "ANTHROPIC_MODEL=mimo-v2.5-pro"]
            cmd += ["-e", "ANTHROPIC_DEFAULT_SONNET_MODEL=mimo-v2.5-pro"]
            cmd += ["-e", "ANTHROPIC_DEFAULT_OPUS_MODEL=mimo-v2.5-pro"]
            cmd += ["-e", "ANTHROPIC_DEFAULT_HAIKU_MODEL=mimo-v2.5-pro"]
            cmd += ["-e", "EXPLORE_AGENT_MODEL=mimo-v2.5-pro"]
        else:
        ...
        # MiMo uses the claude binary under the hood, wrapped in a shell that
        # creates a minimal ~/.claude.json so Claude Code skips onboarding.
        if agent_cmd == "mimo":
            cmd += [config.image, "sh", "-c",
                    'mkdir -p ~/.claude && '
        ...
        if agent_cmd in ("claude", "mimo"):
            cmd += ["--print", "--verbose", "--output-format", "json",
    ...
    if results_dir and preserve_sandbox:
        ...
        if stdout:
            # For claude/mimo agents, output is JSON — extract text for agent_stdout.txt
            # and save token usage separately.
            if agent_cmd in ("claude", "mimo"):
                import json as _json
                text_output = stdout
                token_usage = None
                try:
                    # Output is a JSON array of objects — find the "result" object
                    result_obj = None
                    ...
                    if isinstance(parsed, list):
                        for obj in parsed:
                            if isinstance(obj, dict) and obj.get("type") == "result":
                                result_obj = obj
                    elif isinstance(parsed, dict) and parsed.get("type") == "result":
                        result_obj = parsed
                    if result_obj:
                        text_output = result_obj.get("result", stdout)
                    ...
                    pass
                (dest / "agent_stdout.txt").write_text(text_output, encoding="utf-8")
                if token_usage:
                    (dest / "token_usage.json").write_text(
                        _json.dumps(token_usage, indent=2) + "\n", encoding="utf-8"
                    )
            else:
...
def main() -> None:
    ...
    rp.add_argument("--agent", default="claude", choices=["claude", "codex", "gemini", "vibe", "mimo"])
