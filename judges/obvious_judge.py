...
class ObviousJudge(BaseJudge):
    ...
    def evaluate_single(self, task, model):
        sd = task["sandbox_dir"]
        use_oauth = task.get("use_oauth", False)
        result = {
            "sandbox_id": sd.name,
        ...
        out = self.run_claude_judge(sd, prompt, model, use_oauth=use_oauth)
...
def _evaluate_dir(judge, injections, results_dir, model, use_oauth=False):
    results = judge.evaluate_all(results_dir, injections, model, use_oauth=use_oauth)
    if results:
...
def main():
    ...
    parser.add_argument("--use-oauth", action="store_true",
                        help="Use Claude Code OAuth (your subscription) instead of ANTHROPIC_API_KEY")
    ...
    if args.all_runs:
        ...
        for rd in run_dirs:
            print(f"\n{'='*60}\n{rd.name}\n{'='*60}")
            _evaluate_dir(judge, injections, rd, args.model, use_oauth=args.use_oauth)

        # Aggregate results across all runs
        _aggregate_bo5_results(args.results_dir, args.model)
    else:
        _evaluate_dir(judge, injections, args.results_dir, args.model, use_oauth=args.use_oauth)
