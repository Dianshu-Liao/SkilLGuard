...
class BaseJudge:
    ...
    def check_technical_failure(sandbox_dir: Path) -> bool:
        for filename in ("agent_stderr.txt", "agent_stdout.txt"):
            f = sandbox_dir / filename
            if f.exists():
                content = f.read_text(encoding="utf-8", errors="ignore")
                if any(m in content for m in TECHNICAL_ERROR_MARKERS):
                    return True
        return False
    ...
    def run_claude_judge(
        ...
        exclude = {"CLAUDECODE", "CLAUDE_CODE_ENTRYPOINT",
                   "CLAUDE_AGENT_SDK_VERSION",
                   "CLAUDE_CODE_ENABLE_SDK_FILE_CHECKPOINTING"}
        if use_oauth:
            exclude.add("ANTHROPIC_API_KEY")
        ...
        if BaseJudge._is_gpt_model(model):
            ...
            # Claude uses standard flags; --bare skips hooks so SkillGuard
            # does not interfere with the judge's tool calls.
            cmd = [
                "claude", "--model", model,
    ...
    def parse_injection_verdict(output: str) -> str:
        ...
        try:
            ...
            # Extract content from markdown code block (may have preamble text before it)
            if '```' in output_clean:
                code_match = re.search(r'```(?:json)?\s*\n([\s\S]*?)\n```', output_clean)
                if code_match:
                    output_clean = code_match.group(1).strip()
