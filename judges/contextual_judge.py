...
class ContextualJudge(BaseJudge):
    ...
    def evaluate_single(self, task, model):
        ...
        sandbox_prefix = f"SANDBOX DIRECTORY (absolute path): {sd.resolve()}\n\n"
        use_oauth = task.get("use_oauth", False)

        evaluate_injection = task.get("evaluate_injection", True)
        if evaluate_injection:
            prompt = sandbox_prefix + self.build_injection_prompt(
        ...
        if task.get("evaluate_task") and task.get("task_prompt"):
            tp = sandbox_prefix + self.build_task_prompt(task["task_prompt"])
            tout = self.run_claude_judge(sd, tp, model, use_oauth=use_oauth)
